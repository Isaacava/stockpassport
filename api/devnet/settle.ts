...
const ASSET_MINT_ENV: Record<string, string> = {
  'nvda-demo': 'DEVNET_NVDA_MINT',
  'aapl-demo': 'DEVNET_AAPL_MINT',
  'msft-demo': 'DEVNET_MSFT_MINT',
  'goog-demo': 'DEVNET_GOOG_MINT',
};
...
async function confirmPayment(
  ...
  expectedMint: string,
  user: string,
  expectedSourceDelta: bigint,
  expectedMarketDelta: bigint,
) {
  ...
  if (sourceDelta !== expectedSourceDelta || marketDelta !== expectedMarketDelta) {
    throw new Error('Payment transaction does not match the requested settlement amount and destination');
  }
}

async function transferFromMarket(
  connection: Connection,
  market: Keypair,
  mint: PublicKey,
  ...
  const tx = new Transaction().add(
    createTransferCheckedInstruction(source, mint, destination.address, market.publicKey, rawAmount, decimals),
  );
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!body.side || !body.assetId || !body.wallet || !body.paymentSignature || !body.assetAmount || !body.cashAmountUnits || !body.expiresAt) {
      return Response.json({ error: 'Missing settlement fields' }, { status: 400 });
    }
    ...
    if (body.side === 'buy') {
      ...
      const settlementSignature = await transferFromMarket(connection, market, stockMint, user, assetRaw, 6);
    ...
    const settlementSignature = await transferFromMarket(connection, market, cashMint, user, cashRaw, 6);
    return Response.json({
      ok: true,
      side: body.side,
      assetId: body.assetId,

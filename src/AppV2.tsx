...
const PRICES: Record<string, number> = { NVDA: 175, AAPL: 230, MSFT: 510, GOOG: 250 };
...
export default function AppV2() {
  ...
  const refreshData = useCallback(async (address: string) => {
    ...
    const allocationRule = data.rules.find((rule) => rule.rule_type === 'target_allocations');
  ...
  useEffect(() => { CONNECTION.getEpochInfo().then(() => setRpcStatus('online')).catch(() => setRpcStatus('offline')); }, []);
  useEffect(() => {
    if (!wallet) return;
  ...
  const positions = useMemo(() => knownHoldings.map((holding) => {
    ...
    return { assetId: asset.id, symbol: asset.symbol, valueUsd: Number(holding.amount) * PRICES[asset.referenceSymbol] };
  }), [knownHoldings]);
  ...
  const executeManualTrade = async () => {
    ...
    setBusy(true); setError(null); setMessage(null);
    try { await executeQuote(quote); await Promise.all([refreshChain(wallet!), refreshData(wallet!)]); setQuote(null); setMessage(`${quote.side === 'buy' ? 'Buy' : 'Sell'} confirmed on Devnet.`); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Trade failed.'); await Promise.all([refreshChain(wallet!), refreshData(wallet!)]).catch(() => undefined); }
    finally { setBusy(false); }
  };
  ...
  const executeProposal = async (proposal: RuleProposal) => {
    ...
    const price = PRICES[asset.referenceSymbol];
    ...
    if (!window.confirm(`Authorize ${proposal.kind === 'sell-overweight' ? 'selling' : 'buying'} approximately ${quantity.toFixed(6)} ${asset.referenceSymbol}? Your wallet will sign a real Devnet transaction.`)) return;
    ...
    try {
      ...
      await recordActivity(wallet, { eventType: 'rule_proposal_executed', signature: undefined, payload: { proposalId: proposal.id, kind: proposal.kind, assetId: proposal.assetId, targetPct: proposal.targetPct, valueUsd: proposal.valueUsd } });
      await Promise.all([refreshChain(wallet), refreshData(wallet)]);
      setMessage(`Rule action executed for ${asset.referenceSymbol}. Chain state was refreshed after settlement.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Rule execution failed.'); await Promise.all([refreshChain(wallet), refreshData(wallet)]).catch(() => undefined); }
    finally { setBusy(false); }
  };
  ...
  const saveAllRules = async () => {
    ...
    try {
      const definitions = [
        ['max_allocation', max, undefined],
        ['min_cash_reserve', reserve, undefined],
        ['rebalance_threshold', threshold, undefined],
        ['recurring_contribution', contribution, undefined],
      ] as const;
      ...
      saved.push(await saveRule(wallet, { portfolioId: portfolio.id, ruleType: 'target_allocations', enabled: targetTotal > 0, parameters: { allocations: cleanTargets } }));
  ...
  const renderOverview = () => <div className="grid">
    <div className="hero-card"><div><div className="eyebrow">PORTFOLIO STATE · SOLANA DEVNET</div><div className="hero-number">${evaluation.totalValueUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><p>Portfolio value is derived from confirmed token-account balances. Devnet assets and reference prices are synthetic demo data.</p></div><button className="primary-button" onClick={refreshAll} disabled={busy}>{busy ? 'Working…' : wallet ? 'Refresh state' : 'Connect wallet'}</button></div>
    <div className="metrics"><div className="metric"><span>DEMO-USDC</span><strong>{cashHolding?.amount ?? '0'}</strong><small>Confirmed token balance</small></div><div className="metric"><span>DEVNET SOL</span><strong>{wallet && sol !== null ? sol.toFixed(4) : '—'}</strong><small>Network fee balance</small></div><div className="metric"><span>POSITIONS</span><strong>{knownHoldings.length}</strong><small>On-chain synthetic assets</small></div><div className="metric"><span>ACTIVE RULES</span><strong>{rules.filter((rule) => rule.enabled).length}</strong><small>Persisted in Supabase</small></div></div>
    ...
    <div className="panel"><div className="panel-head"><div><div className="eyebrow">HEALTH</div><h2>{rpcStatus === 'online' ? 'Solana Devnet online' : rpcStatus === 'offline' ? 'RPC unavailable' : 'Checking RPC'}</h2></div><button className="secondary-button" onClick={requestFaucet} disabled={!wallet || funding}>{funding ? 'Funding…' : 'Fund 1,000 DEMO-USDC'}</button></div>{wallet ? <p>{shortAddress(wallet)} · {evaluation.proposals.length} current proposals · {evaluation.violations.length} violations.</p> : <p>Connect a Devnet wallet to read real token balances.</p>}{wallet && <a className="explorer-link" href={explorerAddressUrl(wallet)} target="_blank" rel="noreferrer">Verify wallet on Solana Explorer ↗</a>}</div>
  </div>;

  const renderDiscover = () => { const selected = DEVNET_ASSETS.find((asset) => asset.id === selectedAssetId) ?? DEVNET_ASSETS[0]; return <div className="grid"><div className="panel"><div className="eyebrow">DISCOVER</div><h2>Synthetic tokenized-stock demos</h2><p>These assets exist only on Solana Devnet for the hackathon. They do not represent real-world securities.</p><div className="asset-grid">{DEVNET_ASSETS.map((asset) => <button key={asset.id} className={selectedAssetId === asset.id ? 'asset-card selected' : 'asset-card'} onClick={() => { setSelectedAssetId(asset.id); setQuote(null); }}><span>{asset.referenceSymbol}</span><strong>{asset.symbol}</strong><small>${PRICES[asset.referenceSymbol].toFixed(2)} reference</small></button>)}</div></div><div className="panel trade-panel"><div className="trade-head"><div><div className="eyebrow">EXECUTION TICKET</div><h2>{selected.symbol}</h2></div><div className="side-toggle"><button className={tradeSide === 'buy' ? 'selected-side' : ''} onClick={() => { setTradeSide('buy'); setQuote(null); }}>Buy</button><button className={tradeSide === 'sell' ? 'selected-side' : ''} onClick={() => { setTradeSide('sell'); setQuote(null); }}>Sell</button></div></div><label>Asset quantity<input inputMode="decimal" value={tradeAmount} onChange={(event) => { setTradeAmount(event.target.value); setQuote(null); }} /></label>{quote && <div className="quote-box"><div><span>Reference</span><strong>${quote.referencePriceUsd.toFixed(2)}</strong></div><div><span>Execution</span><strong>${quote.executionPriceUsd.toFixed(2)}</strong></div><div><span>{quote.side === 'buy' ? 'You pay' : 'You receive'}</span><strong>{quote.cashAmount.toFixed(2)} DEMO-USDC</strong></div><div><span>Expires</span><strong>{Math.max(0, Math.ceil((Date.parse(quote.expiresAt) - Date.now()) / 1000))}s</strong></div></div>}<div className="trade-actions"><button className="secondary-button" onClick={requestQuote} disabled={busy}>Get quote</button><button className="primary-button" onClick={executeManualTrade} disabled={!quote || busy}>{busy ? 'Settling…' : `${tradeSide === 'buy' ? 'Buy' : 'Sell'} ${selected.referenceSymbol}`}</button></div></div></div>; };

  const renderPortfolio = () => <div className="grid"><div className="panel"><div className="eyebrow">PORTFOLIO SETTINGS</div><h2>{portfolio?.name ?? 'My Portfolio'}</h2><label>Portfolio name<input value={portfolioName} onChange={(event) => setPortfolioName(event.target.value)} /></label><label>Description<textarea value={portfolioDescription} onChange={(event) => setPortfolioDescription(event.target.value)} /></label><button className="primary-button" onClick={savePortfolioSettings} disabled={busy}>Save portfolio</button></div><div className="panel"><div className="eyebrow">CONFIRMED POSITIONS</div><h2>${evaluation.totalValueUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>{knownHoldings.length === 0 && <p>No positions yet. Fund Demo-USDC and buy an asset from Discover.</p>}{knownHoldings.map((holding) => { const asset = DEVNET_ASSETS.find((item) => item.id === holding.assetId)!; const value = Number(holding.amount) * PRICES[asset.referenceSymbol]; const pct = evaluation.totalValueUsd ? value / evaluation.totalValueUsd * 100 : 0; return <div className="holding-row" key={holding.mint}><div><strong>{asset.symbol}</strong><small>{Number(holding.amount).toLocaleString()} units · {pct.toFixed(1)}% of portfolio</small></div><strong>${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></div>; })}</div></div>;
  ...
  const renderPassport = () => <div className="grid"><div className="hero-card"><div><div className="eyebrow">STOCKPASSPORT</div><h2>Portable portfolio record</h2><p>{wallet ? `Wallet ${shortAddress(wallet)} · ${knownHoldings.length} live positions · ${rules.length} saved rules · ${activities.length} indexed events.` : 'Connect a wallet to build a verifiable record from actual Devnet state.'}</p></div></div><div className="panel"><div className="eyebrow">PASSPORT CONTENT</div><div className="holding-row"><span>Network</span><strong>Solana Devnet</strong></div><div className="holding-row"><span>Ownership source</span><strong>On-chain token accounts</strong></div><div className="holding-row"><span>Configuration source</span><strong>Supabase</strong></div><div className="holding-row"><span>Portfolio</span><strong>{portfolio?.name ?? 'Not created'}</strong></div><div className="holding-row"><span>Current proposals</span><strong>{evaluation.proposals.length}</strong></div>{wallet && <a className="explorer-link" href={explorerAddressUrl(wallet)} target="_blank" rel="noreferrer">Verify wallet on Explorer ↗</a>}</div></div>;
